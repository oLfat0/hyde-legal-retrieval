"""hyde_hybrid_longo.py — RQ3: HyDE + Hybrid, cenario longo"""
...
QUERIES_PATH = Path("data/queries/queries_rq3_longo.json")
CONFIG_NAME  = "hyde_hybrid_rq3_longo"
TOP_K, KS    = 10, [1, 5, 10]
RESULTS_DIR  = "results_rq3"

def run() -> dict:
    print(f"\n{'#'*55}")
    print(f"  RQ3 — HyDE + Hybrid | cenario: longo")
    print(f"  Indice: 501 documentos (RQ3)")
    print(f"  BM25: query original qi")
    print(f"{'#'*55}\n")
    ...
    query_texts = [query_map[cid] for cid in cdac_order]
    ...
    dense_r  = dense_search(mean_emb, index, cdac_list, top_k=TOP_K)
    ...
    results = evaluate(hybrid_r, cdac_order, ks=KS)
