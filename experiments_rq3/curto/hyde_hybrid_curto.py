"""hyde_hybrid_curto.py — RQ3: HyDE + Hybrid, cenario curto"""
from __future__ import annotations
import json, sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from src.retriever_v2      import build_bm25_index, sparse_search, reciprocal_rank_fusion
from src.embedder_v2        import load_model, build_index, dense_search
from src.hyde_embedder_rq3  import run_pipeline
from src.evaluator          import evaluate, print_results, save_results

QUERIES_PATH = Path("data/queries/queries_rq3_curto.json")
CONFIG_NAME  = "hyde_hybrid_rq3_curto"
TOP_K, KS    = 10, [1, 5, 10]
RESULTS_DIR  = "results_rq3"

def run() -> dict:
    print(f"\n{'#'*55}")
    print(f"  RQ3 — HyDE + Hybrid | cenario: curto")
    print(f"  Indice: 501 documentos (RQ3)")
    print(f"  BM25: query original qi")
    print(f"{'#'*55}\n")

    mean_emb, cdac_order = run_pipeline(granul="curto")

    with open(QUERIES_PATH, "r", encoding="utf-8") as f:
        queries = json.load(f)
    query_map   = {q["cdacordao"]: q["query"] for q in queries}
    query_texts = [query_map[cid] for cid in cdac_order]

    model, tokenizer = load_model()
    index, cdac_list = build_index(model, tokenizer)
    bm25, bm25_cdacs = build_bm25_index()
    assert cdac_list == bm25_cdacs, "Ordem FAISS vs BM25 diverge!"

    dense_r  = dense_search(mean_emb, index, cdac_list, top_k=TOP_K)
    sparse_r = sparse_search(query_texts, bm25, cdac_list, top_k=TOP_K)
    hybrid_r = reciprocal_rank_fusion(dense_r, sparse_r, top_k=TOP_K)

    results = evaluate(hybrid_r, cdac_order, ks=KS)
    print_results(results, CONFIG_NAME)
    save_results(results, CONFIG_NAME, output_dir=RESULTS_DIR)
    return results

if __name__ == "__main__":
    run()